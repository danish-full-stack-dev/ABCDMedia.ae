  ...
  const InnerContent = ({ service, reverse }: InnerContentProps) => {
    return (
      ...
      >
        {/* source Section */}
        <div
          className={`w-full md:w-5/12 bg-black/90 md:h-full h-[40vh] md:relative `}
        >
          ...
          service.title.toLocaleLowerCase() === "digital marketing & growth" ? (
            <div className="w-full md:h-full h-[40vh] ">
              <video
                autoPlay
                loop
                muted
          ...
          {allServices.map((servic, index) => {
            ...
            return (
              <motion.div
                ...
                className={`absolute inset-0 w-full md:h-screen bg-white text-black border-t-2`}
              >
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-col gap-6">
          {allServices.map((service, index) => (
            <div
              key={index}
              className="w-full bg-white border-2 border-gray-200 rounded-lg  shadow-sm"
            >
              <div className="flex flex-col">
                {/* Image Section */}
                <div className="w-full h-48 sm:h-64 bg-black/90 relative ">
                  <img
                    src={service.source}
                    alt={service.title}
