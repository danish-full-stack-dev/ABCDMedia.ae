...
<div className="flex flex-col justify-start -mt-6 md:-mt-10">
  ...
  <p className="text-[#a407d9] text-sm uppercase tracking-widest font-semibold mb-4">
    Why Choose Us
  </p>

 {/* Main Heading */} 
 <h2 className="text-3xl md:text-5xl font-bold leading-tight mb-10">
   <span className="md:whitespace-nowrap"> Why Choose{" "} 
    <span className="text-[#a407d9]">ABCDMEDIA?</span> 
    </span> 
    <br /> We grow when you grow. </h2>
        ...
        <div className="flex flex-col justify-start mt-1">
          ...
          <div className="space-y-6">
            {reasons.map((reason) => (
              <div
                key={reason.id}
                className="bg-[#ececf3] rounded-2xl p-6 cursor-pointer transition-all duration-300"
                onClick={() => toggle(reason.id)}
              >
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <div className="bg-[#a407d9] w-7 h-7 rounded-full flex items-center justify-center text-white">
                      <Check size={16} />
                    </div>
                  ...
                  <span className="text-[#a407d9] text-xl">
                    {openId === reason.id ? "↑" : "↓"}
                  </span>
                </div>
